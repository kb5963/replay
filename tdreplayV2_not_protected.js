const	chess = false; // speltype voorlopig even zo
...
const Begin_Pieces      = 20;

const startBoard =
[
	Piece.blackPawn,  // sq_1
	Piece.blackPawn,  // sq_2
	Piece.blackPawn,  // sq_3
	Piece.blackPawn,  // sq_4
	Piece.blackPawn,  // sq_5
 	Piece.blackPawn,  // sq_6
	Piece.blackPawn,  // sq_7
	Piece.blackPawn,  // sq_8
	Piece.blackPawn,  // sq_9
	Piece.blackPawn,  // sq_10
	Piece.blackPawn,  // sq_11
	Piece.blackPawn,  // sq_12
	Piece.blackPawn,  // sq_13
	Piece.blackPawn,  // sq_14
	Piece.blackPawn,  // sq_15
	Piece.blackPawn,  // sq_16
	Piece.blackPawn,  // sq_17
	Piece.blackPawn,  // sq_18
	Piece.blackPawn,  // sq_19
	Piece.blackPawn,  // sq_20
	Piece.empty,      // sq_21
	Piece.empty, 	    // sq_22
	Piece.empty, 	    // sq_23
	Piece.empty, 	    // sq_24
	...
	Piece.empty, 	    // sq_29
	Piece.empty, 	    // sq_30
	Piece.whitePawn,  // sq_31
	Piece.whitePawn,  // sq_32
	Piece.whitePawn,  // sq_33
	Piece.whitePawn,  // sq_34
	Piece.whitePawn,  // sq_35
	Piece.whitePawn,  // sq_36
	Piece.whitePawn,  // sq_37
	Piece.whitePawn,  // sq_38
	Piece.whitePawn,  // sq_39
	Piece.whitePawn,  // sq_40
	Piece.whitePawn,  // sq_41
	Piece.whitePawn,  // sq_42
	Piece.whitePawn,  // sq_43
	Piece.whitePawn,  // sq_44
	Piece.whitePawn,  // sq_45
	Piece.whitePawn,  // sq_46
	Piece.whitePawn,  // sq_47
	Piece.whitePawn,  // sq_48
	Piece.whitePawn,  // sq_49
	Piece.whitePawn,  // sq_50
];
...
[
	...
	sq_6,  sq_7,  sq_8,  sq_9,  sq_10,
	sq_11, sq_12, sq_13, sq_14, sq_15,
	sq_16, sq_17, sq_18, sq_19, sq_20,
	sq_21, sq_22, sq_23, sq_24, sq_25,
	sq_26, sq_27, sq_28, sq_29, sq_30,
	sq_31, sq_32, sq_33, sq_34, sq_35,
	sq_36, sq_37, sq_38, sq_39, sq_40,
	sq_41, sq_42, sq_43, sq_44, sq_45,
	sq_46, sq_47, sq_48, sq_49, sq_50,
];
...
[
	file_2,	file_4,	file_6,	file_8,	file_10,
...
[
	rank_10, rank_10,	rank_10, rank_10,	rank_10,
	rank_9,	 rank_9,	rank_9,	 rank_9,	rank_9,
	rank_8,	 rank_8,	rank_8,	 rank_8,	rank_8,
	rank_7,	 rank_7,	rank_7,	 rank_7,	rank_7,
	rank_6,	 rank_6,	rank_6,	 rank_6,	rank_6,
	rank_5,	 rank_5,	rank_5,	 rank_5,	rank_5,
	rank_4,	 rank_4,	rank_4,	 rank_4,	rank_4,
	rank_3,	 rank_3,	rank_3,	 rank_3,	rank_3,
	rank_2,	 rank_2,	rank_2,	 rank_2,	rank_2,
...
{
 	let value = StrToIntDef(string, 0);
	if (value <= 0)
	{
		return sq_NB;
	}
...
// TO DO bij board tekenen wordt boardRow(square) aangeroepen
// en boardColumn(square)
// die functies wijzigen in rankOf en fileOf zoals bij c++
// boardRow(square) is vervallen, gebruik rankOf
// boardColumn(square) is vervallen, gebruik fileOf

function isBlackPromotionSquare(square)
{
	return square >= sq_46;
}

function isWhitePromotionSquare(square)
{
	return square <= sq_5;
}

function startsWithDarkSquare(rank)
{
	return !(rank % 2);
}
...
forward(move)
{
	...
	{
		this.setPiece(square, Piece.empty);
	}
	this.flipSideToMove();
}
...
backward(move)
{
	...
	this.setPiece(move.to, Piece.empty);
	this.setPiece(move.from, move.fromPiece);
	let i = 0;
	for (const square of move.captured)
	{
		this.setPiece(square, move.capturedPieces[i]);
    i++;
	}
	this.flipSideToMove();
}
...
const BoardPieceEllipse  = 0;
const BoardPieceCircle   = 1;
const DefaultBoardPiece  = BoardPieceEllipse;

function getBoardPieces()
{
  let boardPieces = [];
	boardPieces.push("Ellipse");
	boardPieces.push("Circle");
	return boardPieces;
}
...
captured()
{
  return this.move_.captured.slice();
}

capturedPieces()
{
  return this.move_.capturedPieces.slice();
}
...
copyFrom()
{
	...
	copyGenMove.captured = this.captured.slice();
	copyGenMove.capturedPieces = this.capturedPieces.slice();
	copyGenMove.betweens = this.betweens.slice();
	return copyGenMove;
}
...
isEqual(ref)
{
	...
	{
		for (const square of this.captured)
		{
			let found = ref.captured.find(function(element){ return element == square; });
			if (found === undefined)
			{
				return false;
			}
		}
		for (const piece of this.capturedPieces)
		{
			let found = ref.capturedPieces.find(function(element){ return element == piece; });
			if (found === undefined)
			{
				return false;
			}
...
hasCaptured(square)
{
	let found = this.captured.find(function(element){ return element == square; });
	return (found !== undefined);
}

// deze niet hasCaptured want zelfde naam mag niet
hasCapturedSquares(squares)
{
...
// in c++ heet deze hasBetweens wat een vreemde naam is
hasBetween(square)
{
	let found = this.betweens.find(function(element){ return element == square; });
	return (found !== undefined);
}
...
const genBoard =
[
	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB, sq_NB, sq_NB,	sq_NB, sq_NB, sq_NB, sq_NB, sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_NB, sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_1,	sq_NB, sq_2,	sq_NB, sq_3,	sq_NB, sq_4,	sq_NB, sq_5,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_6,  sq_NB,	sq_7,  sq_NB,	sq_8,  sq_NB,	sq_9,  sq_NB,	sq_10, sq_NB,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_11,	sq_NB, sq_12,	sq_NB, sq_13,	sq_NB, sq_14,	sq_NB, sq_15,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_16, sq_NB,	sq_17, sq_NB,	sq_18, sq_NB,	sq_19, sq_NB,	sq_20, sq_NB,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_21,	sq_NB, sq_22,	sq_NB, sq_23,	sq_NB, sq_24,	sq_NB, sq_25,	sq_NB, sq_NB,
	sq_NB, sq_NB, sq_26, sq_NB,	sq_27, sq_NB,	sq_28, sq_NB,	sq_29, sq_NB,	sq_30, sq_NB,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_31,	sq_NB, sq_32,	sq_NB, sq_33,	sq_NB, sq_34,	sq_NB, sq_35,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_36, sq_NB,	sq_37, sq_NB,	sq_38, sq_NB,	sq_39, sq_NB,	sq_40, sq_NB,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_41,	sq_NB, sq_42,	sq_NB, sq_43,	sq_NB, sq_44,	sq_NB, sq_45,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_46, sq_NB,	sq_47, sq_NB,	sq_48, sq_NB,	sq_49, sq_NB,	sq_50, sq_NB,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,
	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,	sq_NB, sq_NB,
];
...
{
	this.genMoves_ = [];
  this.captured_ = Array(Square_Size).fill(sq_NB);
  this.betweens_ = Array(Square_Size).fill(sq_NB);
  this.capturedPieces_ = Array(Square_Size).fill(Piece.empty);
  this.blocked_ = Array(Square_Size).fill(false);
  this.maxCapture_ = 0;
	this.position_ = new GamePosition();
	this.numberOfCaptures_ = 0;
	this.from_ = sq_NB;
	this.fromPiece_ = Piece.empty;
	this.northWest_ = 0;
	this.northEast_ = 0;
	this.southWest_ = 0;
	this.southEast_ = 0;
...
{
	this.position_ = position.copyFrom();
	this.blocked_.fill(false);
  this.maxCapture_ = 0;
	let dimension = 14;
	if (this.position_.isWhiteToMove())
	{
		this.northWest_ = -(dimension + 1);
		this.northEast_ = -(dimension - 1);
		this.southWest_ = (dimension - 1);
		this.southEast_ = (dimension + 1);
	}
	else
	{
		this.northWest_ = (dimension + 1);
...
generateMovesFromSquare(from)
{
 	this.from_ = from;
	this.fromPiece_ = this.position_.at(this.from_);
	this.numberOfCaptures_ = 0;
	if (
		(this.position_.isWhiteToMove() && this.fromPiece_ == Piece.whitePawn) ||
		(this.position_.isBlackToMove() && this.fromPiece_ == Piece.blackPawn))
	{
		this.generateManMoveFromSquare(this.from_);
	}
	else if (
		(this.position_.isWhiteToMove() && this.fromPiece_ == Piece.whiteKing) ||
		(this.position_.isBlackToMove() && this.fromPiece_ == Piece.blackKing))
	{
		this.generateKingMoveFromSquare(this.from_);
	}
}

generateManMoveFromSquare(square)
{
 	this.generateManMove(square, this.northWest_);
	this.generateManMove(square, this.northEast_);
}

generateManMove(from, direction)
{
	let to = genBoard[genMap[from] + direction];
	if (to != sq_NB)
  {
    if (this.position_.isEmpty(to))
  	{
	  	this.addMove(to);
  	}
  }
...
generateKingMoveFromSquare(from)
{
	this.generateKingMove(from, this.northWest_);
	this.generateKingMove(from, this.northEast_);
	this.generateKingMove(from, this.southEast_);
	this.generateKingMove(from, this.southWest_);
}

generateKingMove(from, direction)
{
	let delta = direction;
	do
	{
		let to = genBoard[genMap[from] + delta];
		if (to == sq_NB)
    {
      break;
    }
...
generateCapturesFromSquare(from)
{
	this.from_ = from;
	this.fromPiece_ = this.position_.at(this.from_);
	this.numberOfCaptures_ = 0;
	let piece = this.position_.at(this.from_);
	if ((this.position_.isWhiteToMove() && this.fromPiece_ == Piece.whitePawn) ||
		(this.position_.isBlackToMove() && this.fromPiece_ == Piece.blackPawn))
	{
		this.position_.setPiece(this.from_, Piece.empty);
		this.generateManCaptureFromSquare(this.from_);
	}
	else if (
		(this.position_.isWhiteToMove() && this.fromPiece_ == Piece.whiteKing) ||
		(this.position_.isBlackToMove() && this.fromPiece_ == Piece.blackKing))
	{
		this.position_.setPiece(this.from_, Piece.empty);
		this.generateKingCaptureFromSquare(this.from_);
	}
	this.position_.setPiece(this.from_, piece);
}

generateManCaptureFromSquare(from)
{
	let isTo = true;
	if (this.generateManCapture(from, this.northWest_))
	{
		isTo = false;
	}
	if (this.generateManCapture(from, this.northEast_))
	{
		isTo = false;
	}
	if (this.generateManCapture(from, this.southEast_))
	{
		isTo = false;
	}
	if (this.generateManCapture(from, this.southWest_))
	{
		isTo = false;
	}
	if (isTo)
	{
		this.addCapture(from);
	}
}

generateManCapture(from, direction)
{
	let capture = genBoard[genMap[from] + direction];
	if (capture == sq_NB)
	{
		return false;
	}
	let to = genBoard[genMap[from] + 2 * direction];
	if (to == sq_NB)
	{
		return false;
	}
	...
	if (this.blocked_[capture])
	{
		return false;
	}
	if (this.fromPiece_ == Piece.whitePawn)
	{
		if (!this.position_.isBlack(capture))
		{
			return false;
		}
	}
	...
	{
		if (!this.position_.isWhite(capture))
		{
			return false;
		}
	}
	this.captured_[this.numberOfCaptures_] = capture;
	this.capturedPieces_[this.numberOfCaptures_] = this.position_.at(capture);
	this.betweens_[this.numberOfCaptures_] = to;
	this.numberOfCaptures_++;
	let piece = this.position_.at(capture);
	this.blocked_[capture] = true;
	this.generateManCaptureFromSquare(to);
	this.blocked_[capture] = false;
	this.position_.setPiece(capture, piece);
	this.numberOfCaptures_--;
	return true;
}

generateKingCaptureFromSquare(from)
{
	let isTo = true;
	if (this.generateKingCapture(from, this.northWest_))
	{
		isTo = false;
	}
	if (this.generateKingCapture(from, this.northEast_))
	{
		isTo = false;
	}
	if (this.generateKingCapture(from, this.southEast_))
	{
		isTo = false;
	}
	if (this.generateKingCapture(from, this.southWest_))
	{
		isTo = false;
	}
	if (isTo)
...
generateKingCapture(begin, direction)
{
	let capture = sq_NB;
	let delta = direction;
	do
	{
		capture = genBoard[genMap[begin] + delta];
		if (capture == sq_NB)
		{
			return false;
		}
		...
		if (this.fromPiece_ == Piece.whiteKing)
		{
			if (this.position_.isBlack(capture))
			{
				break;
		...
		if (!this.position_.isEmpty(capture))
		{
			return false;
		}
		delta += direction;
	...
	{
		delta += direction;
		let to = genBoard[genMap[begin] + delta];
		if (to == sq_NB)
    {
      return ret;
    }
    if (!this.position_.isEmpty(to))
		...
		this.betweens_[this.numberOfCaptures_] = to;
		this.numberOfCaptures_++;
		let tempPiece = this.position_.at(capture);
		this.blocked_[capture] = true;
		this.generateKingCaptureFromSquare(to);
		this.blocked_[capture] = false;
		this.position_.setPiece(capture, tempPiece);
		this.numberOfCaptures_--;
		ret = true;
	} while (true);
}
...
addCapture(to)
{
	if (!this.numberOfCaptures_)
	{
		return;
	}
	if (this.numberOfCaptures_ >= this.maxCapture_)
	{
		if (this.numberOfCaptures_ > this.maxCapture_)
		{
      this.genMoves_.length = 0;
		}
		this.addMove(to);
	}
}
...
addMove(to)
{
	...
	move.from = this.from_;
	move.to = to;
	move.fromPiece = this.fromPiece_;
	move.toPiece = move.fromPiece;
	if (move.fromPiece == Piece.whitePawn)
	{
		if (isWhitePromotionSquare(move.to))
		{
			move.toPiece = Piece.whiteKing;
		}
	}
	else if (move.fromPiece == Piece.blackPawn)
	{
		if (isBlackPromotionSquare(move.to))
		{
			move.toPiece = Piece.blackKing;
		}
	}
	for (let i = 0; i < this.numberOfCaptures_; i++)
	{
		move.captured.push(this.captured_[i]);
		move.capturedPieces.push(this.capturedPieces_[i]);
	}
	for (let i = 0; i < this.numberOfCaptures_ - 1; i++)
	{
		move.betweens.push(this.betweens_[i]);
	}

	if (this.unique_)
	{
		if (this.numberOfCaptures_ >= 2)
		{
			let found = this.genMoves_.find(function(element)
			{
				if (element.isEqual(move))
				{
					return true;
				}
			});
	...
	this.maxCapture_ = move.numberOfCaptures();
	this.genMoves_.push(move);
}

moves(position, unique)
{
	...
	for (const square of boardSquares)
	{
		this.generateCapturesFromSquare(square);
	}

	if (this.genMoves_.length === 0)
	{
		for (const square of boardSquares)
		{
			this.generateMovesFromSquare(square);
		}
	}
	return this.genMoves_;
}
...
canCapture(position)
{
	...
	return this.genMoves_.length > 0;
}

canCaptureSquare(position, capture)
{
	this.initPosition(position);
	this.unique_ = true;
	this.genMoves_.length = 0;
	...
	for (const genMove of this.genMoves_)
	{
		if (genMove.hasCaptured(capture))
		{
			return true;
		}
	}
...
betweens(position, move)
{
	this.initPosition(position);
	this.unique_ = true;
	this.genMoves_.length = 0;

	this.generateCapturesFromSquare(move.from);
	let testCaptured = [];
  let captured = move.captured;
	for (const capture of captured)
	{
		testCaptured.push(capture);
	}
	for (const genMove of this.genMoves_)
	{
		if (genMove.to != move.to)
		{
			continue;
		}
		if (!genMove.hasCapturedSquares(testCaptured))
		{
			continue;
		}
		return genMove.betweens.slice();
	}
	return [];
}

isAmbiguous(position, move)
{
	this.initPosition(position);
	this.unique_ = true;
	this.genMoves_.length = 0;
...
{
	let gen = new MoveGenerator();
	return gen.moves(position, true);
}

function generateFromMoves(position, from)
{
	let gen = new MoveGenerator();
	let genMoves = gen.moves(position, true);
	let genFromMoves = [];
...
function generateFromToCapturedMoves(position, from, to, captured)
{
	let gen = new MoveGenerator();
	let genMoves = gen.moves(position, true);
	let moves = [];
	for (const genMove of genMoves)
	{
		...
		if (!genMove.hasCaptured(captured))
		{
			continue;
		}
		moves.push(genMove);
...
function generateFromToBetweenMoves(position, from, to, betweens)
{
	let gen = new MoveGenerator();
	let genMoves = gen.moves(position, false);
	let moves = [];
	for (const genMove of genMoves)
	{
		if (genMove.from != from)
		...
		if (!genMove.hasBetweens(betweens))
		{
			continue;
		}
		// don't allow doubles
...
function generateBetweens(position, move)
{
	let gen = new MoveGenerator();
	return gen.betweens(position, move);
}

function canCapture(position)
{
	let gen = new MoveGenerator();
	return gen.canCapture(position);
}

function canCaptureSquare(position, capture)
{
	let gen = new MoveGenerator();
	return gen.canCaptureSquare(position, capture);
}

function isAmbiguous(position, move)
{
	let gen = new MoveGenerator();
	return gen.isAmbiguous(position, move);
}

function isStable(position)
{
	let gen = new MoveGenerator();
	if (gen.canCapture(position))
	{
		return false;
	}
	let flipPosition = position.copyFrom();
	flipPosition.flipSideToMove();
	return !gen.canCapture(flipPosition);
}
...
function moveToString(position, move)
{
 	let string = '';
	if (move.empty())
	{
  ...
  else
  {
    ...
    {
      if (isAmbiguous(position, move))
      {
        let betweens = generateBetweens(position, move);
        for (const square of betweens)
        {
          string += squareToAlphaNumericString(square);
          string += 'x';
        }
    ...
    for (let rank = 0; rank < Board_Size; rank++)
		{
			...
			let top = boardRect.Top + 1 + (rank + 1) * boardProperties.squareWidth;
    ...
    for (let file = 0; file < Board_Size; file++)
		{
			...
			let left = boardRect.Left + 1 + (file + 1) * boardProperties.squareWidth;
      let textWidth = ctx.measureText(nr).width;
      ...
      ctx.fillText(nr, textLeft, textTop);
			if (flipped)
			{
				nr--;
			}
...
function drawEllipsePiece(ctx, rect, piece)
{
 	ctx.lineWidth = "1";
  ...
  let pieceProportion = 80;
  let squareSize = rect.Width() + 1;
 	let offset = ((0.19 * pieceProportion) / 100) * squareSize;
  let middleX = rect.Left + squareSize / 2;
  let radiusX = (squareSize / 4);
  let radiusY = (squareSize / 2.4);
	if (piece == Piece.whitePawn || piece == Piece.blackPawn)
	{
    let middleY = rect.Top + squareSize / 1.8;
    ctx.beginPath();
    ctx.ellipse(middleX, middleY, radiusX, radiusY, Math.PI / 2, 0, 2 * Math.PI);
    ctx.fill();
    ctx.stroke();
		middleY -= offset;
    ctx.beginPath();
	...
	{
    let middleY = rect.Top + squareSize / 1.55;
    ctx.beginPath();
    ctx.ellipse(middleX, middleY, radiusX, radiusY, Math.PI / 2, 0, 2 * Math.PI);
    ctx.fill();
    ctx.stroke();
...
function drawCirclePiece(ctx, rect, piece)
{
 	ctx.lineWidth = "1";
  if (piece == Piece.whitePawn || piece == Piece.whiteKing)
  {
  ...
  let middleY = rect.Top + squareSize / 2; // new
...
function drawPiece(ctx, rect, piece)
{
  ...
  if (globalBoardProperties.pieces == 0)
  {
    drawEllipsePiece(ctx, rect, piece);
  }
  else
  {
    drawCirclePiece(ctx, rect, piece);
  }
}
...
function portableFenToPosition(element)
{
	let position = new GamePosition();
	let len = element.length;
	if (len < 6) // at least "W:W:B."
	{
		return position;
	}
	if (element[0] != 'W' && element[0] != 'B')
	{
		return position;
	}
	if (element[0] == 'W')
	...
	let whitePos = element.indexOf(":W");
	let blackPos = element.indexOf(":B");
	let blackEndPos = element.indexOf(".");
  if (blackEndPos == -1)
	{
		blackEndPos = element.length;
	}
	if (whitePos == -1 || blackPos == -1 || blackEndPos <= blackPos)
	{
		return position;
	}
	let w = element.substr(whitePos + 2, blackPos - (whitePos + 2));
	let b = element.substr(blackPos + 2, blackEndPos - (blackPos + 2));
	let whiteSquares = w.split(",");
	let blackSquares = b.split(",");

	for (const s of whiteSquares)
	{
		if (s.length)
		{
			if (s[0] == 'K')
			{
				if (s.length > 1)
				{
					let sking = s.substr(1, 2);
					let square = StrToIntDef(sking, 0);
					if (square)
					{
						position.setPiece(square-1, Piece.whiteKing);
					}
				}
			}
			else
			{
				let square = StrToIntDef(s, 0);
				if (square)
				{
					position.setPiece(square-1, Piece.whitePawn);
				}
			}
		}
	}
	for (const s of blackSquares)
	{
		if (s.length)
		{
			if (s[0] == 'K')
			{
				if (s.length > 1)
				{
					...
					if (square)
					{
						position.setPiece(square-1, Piece.blackKing);
					}
				}
			}
			else
			{
				...
				if (square)
				{
					position.setPiece(square-1, Piece.blackPawn);
				}
			}
		}
	}
...
function portableToMove(position, astring)
{
  let slen = astring.length;
	...
	while (i < len)
	{
		...
		let stringSquare = astring.substr(start, count);
    squares.push(alphaNumericStringToSquare(stringSquare));
		i++; // skip '-' or 'x' or ':'
	}
	...
	else if (numberOfSquares == 2)
	{
		let from = squares[0];
		let to = squares[1];
		let moves = generateFromToMoves(position, from, to);
		if (moves.length == 1)
		{
			return moves[0];
		}
	...
	else
	{
		...
		let to = squares[1];
		let captured = [];
		for (let i = 2; i < numberOfSquares; i++)
		{
			captured.push(squares[i]);
		}
		let moves = generateFromToCapturedMoves(position, from, to, captured);
		if (moves.length == 1)
		{
			return moves[0];
		}
		...
		{
			betweens.push(squares[i]);
		}
		moves = generateFromToBetweenMoves(position, from, to, betweens);
		if (moves.length == 1)
		{
			return moves[0];
		}
...
buildEngine()
{
  this.engineThread_ = new Worker("/sharedgames/js/stockfish-17.1-8e4d048.js");
...
drawCoordinates()
{
	...
	boardRect.SetHeight(Board_Size * squareWidth + 2 * borderWidth);
 	...
 	let ctx = this.getCanvasContext();
  let coordinateMetrics = calcCoordinateMetrics(ctx, marginWidth-8);
