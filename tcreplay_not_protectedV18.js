...
async buildEngine() {
    var self = this;
    this.engineStarted = false; // The Gatekeeper
    this.ready_ = false;        // Ensure we start fresh
    ...
    try {
        ...
        this.engineThread_ = new Worker(blobUrl);

        this.engineThread_.onmessage = (event) => {
            const line = event.data;
            if (typeof line === 'string' && line.includes("Stockfish") && !this.engineStarted) {
...
init() {
    ...
    // 1. Set standard options
    for (const param of this.engineItem_.params.params) {
        this.toEngine("setoption name " + param.name + " value " + param.value);
    }

    // 2. Set the Variant (Crucial for Stockfish 10 Multi-Variant)
    this.toEngine("setoption name UCI_Variant value chess");

    // 3. Ask if it is ready
...
doFromEngine(line) {
    ...
    // The Engine says "I am ready to think"
    if (command == "readyok") {
        console.log("System: Engine is READY");
        this.ready_ = true;
        if (this.onmessage) {
            this.onmessage({ cmd: "ready", data: {} });
        }
        return;
    }

    if (command == "uciok") {
        this.handleWaitCommand(line); // Calls init()
    }
    else if (command == "bestmove") {
        this.handleDoneCommand(line);
    }
    // ... rest of your else if (id, info, option) stays the same ...
  	else if (command == "id")
	  {
  		this.handleIdCommand(line);
	  }
...
handleIdCommand(line)
{
  ...
  // Fix: Just to be safe with Stockfish's spacing
  if (word2 == "name")
  {
    this.engineId_.name = value.trim();
  }
  else if (word2 == "author")
  {
    this.engineId_.author = value.trim();
  }
}
