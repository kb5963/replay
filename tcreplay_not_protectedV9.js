...
async buildEngine() {
    var self = this;
    // 1. Define the home base for your files
    var folderUrl = "https://kb5963.github.io/my-web-files/engines/stockfish/";
    var engineJsUrl = folderUrl + "stockfish-17.1-8e4d048.js";

    console.log("System: Starting GitHub Engine Assembly...");

    try {
        const response = await fetch(engineJsUrl);
        if (!response.ok) throw new Error("Could not reach GitHub JS file.");
        const scriptText = await response.text();

        // 2. THE OVERRIDE BLOCK
        // This stops the "ERR_NAME_NOT_RESOLVED" by forcing the internal
        // Stockfish variables (o, t, s) to use your GitHub paths.
        var pathFix =
            "// Manually override internal Stockfish path logic\n" +
            "var folder = '" + folderUrl + "';\n" +
            "var o = folder + 'stockfish-17.1-8e4d048.wasm';\n" +
            "var t = '';\n" +
            "var s = folder + 'stockfish-17.1-8e4d048.js';\n" +
            "var Module = { locateFile: function(path) { return folder + path; } };\n" +
            "console.log('Worker: Internal paths redirected to GitHub');\n";

        // 3. Create the "Secure Package" (Blob)
        ...
        // 5. Listener for messages back from the engine
        self.h2x.onmessage = function(event) {
            // This will help you see if the engine wakes up in the console
            if (typeof event.data === 'string' && event.data.includes('Stockfish')) {
                console.log("System: Engine is ONLINE.");
            }
            self.o5f(event); // Calls your existing 'fromEngine' logic
        };

        self.h2x.onerror = function(err) {
            console.error("Worker Error:", err.message);
        };

    } catch (error) {
        console.error("System: buildEngine failed ->", error);
    }
}
